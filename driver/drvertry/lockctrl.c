...
Abstract:
    ...
    This module implements the Lock Control routines for Xfs called
...
#define BugCheckFileId                   (XFS_BUG_CHECK_LOCKCTRL)

#ifdef ALLOC_PRAGMA
#pragma alloc_text(PAGE, XCommonLockControl)
#pragma alloc_text(PAGE, XFastLock)
#pragma alloc_text(PAGE, XFastUnlockAll)
#pragma alloc_text(PAGE, XFastUnlockAllByKey)
#pragma alloc_text(PAGE, XFastUnlockSingle)
#endif
...

{
    ...
    if (Fcb->FileLock == NULL) { XCreateFileLock( IrpContext, Fcb, TRUE ); }
    ...
    XCompleteRequest( IrpContext, NULL, Status );
    return Status;
}
...

{
    ...
    TypeOfOpen = XFastDecodeFileObject( FileObject, &Fcb );
    ...
    if (!XVerifyFcbOperation( NULL, Fcb )) {
    ...
    try {
        ...
        if ((Fcb->FileLock == NULL) && !XCreateFileLock( NULL, Fcb, FALSE )) {
