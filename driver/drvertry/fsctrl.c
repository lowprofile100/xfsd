...
Abstract:
    ...
    This module implements the File System Control routines for Xfs called
...
#define BugCheckFileId                   (XFS_BUG_CHECK_FSCTRL)

//
//  Local constants
//

BOOLEAN XDisable = FALSE;
...
#ifdef ALLOC_PRAGMA
#pragma alloc_text(PAGE, XCommonFsControl)
#pragma alloc_text(PAGE, XDismountVolume)
#pragma alloc_text(PAGE, XFindActiveVolDescriptor)
#pragma alloc_text(PAGE, XFindPrimaryVd)
#pragma alloc_text(PAGE, XIsPathnameValid)
#pragma alloc_text(PAGE, XIsRemount)
#pragma alloc_text(PAGE, XIsVolumeDirty)
#pragma alloc_text(PAGE, XIsVolumeMounted)
#pragma alloc_text(PAGE, XLockVolume)
#pragma alloc_text(PAGE, XMountVolume)
#pragma alloc_text(PAGE, XOplockRequest)
#pragma alloc_text(PAGE, XScanForDismountedVcb)
#pragma alloc_text(PAGE, XUnlockVolume)
#pragma alloc_text(PAGE, XUserFsctl)
#pragma alloc_text(PAGE, XVerifyVolume)
#endif
...

{
    ...
    XPurgeVolume( IrpContext, Vcb, FALSE );
    ...
    XAcquireVcbExclusive( IrpContext, Vcb, FALSE );
    ...
    if (!FlagOn( Vcb->Vpb->Flags, VPB_LOCKED ) && 
        ...
        (Vcb->VcbUserReference == XFS_RESIDUAL_USER_REFERENCE + RemainingUserReferences))  {
...

{
    ...
    case IRP_MN_USER_FS_REQUEST:
        ...
        Status = XUserFsctl( IrpContext, Irp );
        break;

    case IRP_MN_MOUNT_VOLUME:

        Status = XMountVolume( IrpContext, Irp );
        break;

    case IRP_MN_VERIFY_VOLUME:

        Status = XVerifyVolume( IrpContext, Irp );
        break;
...

{
    ...
    case FSCTL_REQUEST_FILTER_OPLOCK :
        ...
        Status = XOplockRequest( IrpContext, Irp );
        break;

    case FSCTL_LOCK_VOLUME :

        Status = XLockVolume( IrpContext, Irp );
        break;

    case FSCTL_UNLOCK_VOLUME :

        Status = XUnlockVolume( IrpContext, Irp );
        break;

    case FSCTL_DISMOUNT_VOLUME :

        Status = XDismountVolume( IrpContext, Irp );
        break;

    case FSCTL_IS_VOLUME_DIRTY :

        Status = XIsVolumeDirty( IrpContext, Irp );
        break;

    case FSCTL_IS_VOLUME_MOUNTED :

        Status = XIsVolumeMounted( IrpContext, Irp );
        break;

    case FSCTL_IS_PATHNAME_VALID :

        Status = XIsPathnameValid( IrpContext, Irp );
        break;

    case FSCTL_INVALIDATE_VOLUMES :

        Status = XInvalidateVolumes( IrpContext, Irp );
        break;
    ...
    )
{
    ...
    XUpdateVcbCondition( OldVcb, VcbMounted);
    XUpdateMediaChangeCount( OldVcb, NewVcb->MediaChangeCount);
...
Routine Description:
    ...
    Its job is to verify that the volume denoted in the IRP is a Xrom volume,
    ...
    2. Read the disk and check if it is a Xrom volume.

    3. If it is not a Xrom volume then delete the Vcb and
...

{
    ...
    //
    //  Check that we are talking to a Xrom device.  This request should
    //  always be waitable.
    //

    ASSERT( Vpb->RealDevice->DeviceType == FILE_DEVICE_X_ROM );
    ...
    SetDoVerifyOnFail = XRealDevNeedsVerify( IrpContext->RealDevice);
    ...
    if (XDisable) {
        ...
        XCompleteRequest( IrpContext, Irp, STATUS_UNRECOGNIZED_VOLUME );
    ...
    Status = XPerformDevIoCtrl( IrpContext,
                                 IOCTL_XROM_CHECK_VERIFY,
    ...
    try {
        ...
        XromToc = FsRtlAllocatePoolWithTag( XPagedPool,
                                             sizeof( XROM_TOC ),
                                             TAG_XROM_TOC );

        RtlZeroMemory( XromToc, sizeof( XROM_TOC ));

        //
        //  Do a quick check to see if there any Vcb's which can be removed.
        //

        XScanForDismountedVcb( IrpContext );

        //
        //  Get our device object and alignment requirement.
        //

        Status = IoCreateDevice( XData.DriverObject,
        ...
        XMarkRealDevVerifyOk( Vcb->Vpb->RealDevice);
        ...
        if (!FlagOn( Vcb->VcbState, VCB_STATE_AUDIO_DISK))  {
            ...
            RawIsoVd = FsRtlAllocatePoolWithTag( XNonPagedPool,
            ...
            FoundPvd = XFindPrimaryVd(   IrpContext,
                                          Vcb,
                                          RawIsoVd,
                                          BlockFactor,
                                          TRUE,
            ...
            if (!FoundPvd)  {
                ...
                //  audio tracks on this disc,  so we'll try to mount it as an audio X.
                ...
                if (!(ToXiskFlags & XROM_DISK_AUDIO_TRACK) ||
                     BooleanFlagOn( Vcb->XromToc->TrackData[0].Control, TOC_DATA_TRACK))  {
                
                    try_leave( Status = STATUS_UNRECOGNIZED_VOLUME);
                }

                SetFlag( Vcb->VcbState, VCB_STATE_AUDIO_DISK | VCB_STATE_XXA );
        ...
        if (FoundPvd) {
            ...
            XFindActiveVolDescriptor( IrpContext,
                                       Vcb,
                                       RawIsoVd,
                                       FALSE);
        }
        ...
        if (XIsRemount( IrpContext, Vcb, &OldVcb )) {
        ...
        XUpdateVcbFromVolDescriptor( IrpContext,
                                      Vcb,
                                      RawIsoVd );
        ...
        Vcb->VcbReference -= XFS_RESIDUAL_REFERENCE;
        ASSERT( Vcb->VcbReference == XFS_RESIDUAL_REFERENCE );

        ObDereferenceObject( Vcb->TargetDeviceObject );

        XUpdateVcbCondition( Vcb, VcbMounted);
        ...
        if ((AbnormalTermination() || (Status != STATUS_SUCCESS)) && 
            ...
            XMarkRealDevForVerify( IrpContext->RealDevice);
        }
        ...
        if (Vcb != NULL) {
            ...
            if (XDismountVcb( IrpContext, Vcb )) {
...

{
    ...
    //
    //  We check that we are talking to a Xrom device.
    //
    ...
    try {
        ...
        if (MediaChangeCount == 0 ||
            ...
            //  a TOC read on,  for example,  erased X-RW media.
            //
            ...
            if (Status != STATUS_SUCCESS) {
                ...
                if (!XIsRawDevice( IrpContext, Status )) {
            ...
            //
                       ...
                       (Vcb->DiskFlags != ToXiskFlags) ||
                       !RtlEqualMemory( XromToc,
                                        Vcb->XromToc,
                                        TocLength )) {
            ...
            if (!FlagOn( Vcb->VcbState, VCB_STATE_AUDIO_DISK )) {
                ...
                if (!XFindPrimaryVd( IrpContext,
                    ...
                    if (Vpb->SerialNumber != XSerial32( RawIsoVd, SECTOR_SIZE )) {
                    ...
                    if (!FlagOn( Vcb->VcbState, VCB_STATE_JOLIET )) {
                        ...
                        AnsiLabel.Buffer = XRvdVolId( RawIsoVd, Vcb->VcbState );
                    ...
                    //
                        ...
                        XConvertBigToLittleEndian( IrpContext,
                                                    XRvdVolId( RawIsoVd, Vcb->VcbState ),
                                                    VOLUME_ID_LENGTH,
                                                    (PCHAR) VolumeLabel );
        ...
        XMarkRealDevVerifyOk( Vpb->RealDevice);
    ...
    try_exit: NOTHING;
        ...
        XUpdateMediaChangeCount( Vcb, MediaChangeCount);
        ...
        if (Status == STATUS_WRONG_VOLUME) {
            ...
            XUpdateVcbCondition( Vcb, VcbNotMounted);
            ...
            if (Vcb->VcbCleanup == 0) {
                ...
                if (NT_SUCCESS( XPurgeVolume( IrpContext, Vcb, FALSE ))) {
                    
                    ReleaseVcb = XCheckForDismount( IrpContext, Vcb, FALSE );
                }
            }
        }
...

{
    ...
    if (XDecodeFileObject( IrpContext,
                            IrpSp->FileObject,
                            &Fcb,
                            &Ccb ) != UserFileOpen ) {
    ...
    case FSCTL_REQUEST_FILTER_OPLOCK :
        ...
        XAcquireFcbExclusive( IrpContext, Fcb, FALSE );
    ...
    case FSCTL_OPLOCK_BREAK_ACK_NO_2:
        ...
        XAcquireFcbShared( IrpContext, Fcb, FALSE );
        break;
    ...
    try {
        ...
        XLockFcb( IrpContext, Fcb );
        Fcb->IsFastIoPossible = XIsFastIoPossible( Fcb );
        XUnlockFcb( IrpContext, Fcb );
...

{
    ...
    if (XDecodeFileObject( IrpContext, IrpSp->FileObject, &Fcb, &Ccb ) != UserVolumeOpen) {
    ...
    try {
        ...
        XVerifyVcb( IrpContext, Vcb );

        Status = XLockVolumeInternal( IrpContext, Vcb, IrpSp->FileObject );
...

{
    ...
    Status = XUnlockVolumeInternal( IrpContext, Vcb, IrpSp->FileObject );
...

{
    ...
    if (Vcb->VcbCondition != VcbMounted) {
        ...
        if (Vcb->VcbCondition != VcbDismountInProgress) {
            ...
            XUpdateVcbCondition( Vcb, VcbInvalid);
        }
...

{
    ...
    if (Irp->AssociatedIrp.SystemBuffer != NULL) {
        ...
        XCompleteRequest( IrpContext, Irp, STATUS_INVALID_USER_BUFFER );
    ...
    *VolumeState = 0;
    ...
    if (Fcb->Vcb->VcbCondition != VcbMounted) {
        ...
        XCompleteRequest( IrpContext, Irp, STATUS_VOLUME_DISMOUNTED );
        return STATUS_VOLUME_DISMOUNTED;
    }

    //
    //  Now set up to return the clean state.  Xs obviously can never be dirty
...

{
    ...
    XDecodeFileObject( IrpContext, IrpSp->FileObject, &Fcb, &Ccb );
...
Routine Description:
    ...
    This routine determines if pathname is a valid XFS pathname.
...

{
    ...
    if (!SeSinglePrivilegeCheck( TcbPrivilege, Irp->RequestorMode )) {
        ...
        XCompleteRequest( IrpContext, Irp, STATUS_PRIVILEGE_NOT_HELD );
    ...
    Links = XData.VcbQueue.Flink;

    while (Links != &XData.VcbQueue) {
...

{
    ...
    while (Links != &XData.VcbQueue) {
        ...
        if ((Vcb->VcbCondition == VcbDismountInProgress) ||
            ...
            ((Vcb->VcbCondition == VcbNotMounted) && (Vcb->VcbReference <= XFS_RESIDUAL_REFERENCE))) {
...

{
    ...
    //
    //  This explicitly breaks various non-BlueBook compliant Xs that scribble
    //  an ISO filesystem on media claiming only audio tracks.  Since these
    //  disks can cause serious problems in some XROM units, fail fast.  I admit
    ...
    while (!FoundVd && (ThisPass <= 2)) {
        ...
        if (ThisPass == 1) {
            ...
            try {
                ...
                if (NT_SUCCESS( Status ) &&
                    (XromToc->FirstTrack != XromToc->LastTrack)) {
                    ...
                    Source = (PUCHAR) &XromToc->TrackData[0].Address[3];
                ...
                if (XromToc != NULL) { ExFreePool( XromToc ); }
            }
        }
        ...
        while (TRUE) {
            ...
            if (RtlEqualMemory( XIsoId,
                                XRvdId( RawIsoVd, VCB_STATE_ISO ),
                                VOL_ID_LEN )) {

                SetFlag( VolumeFlags, VCB_STATE_ISO );

            } else if (RtlEqualMemory( XHsgId,
                                       XRvdId( RawIsoVd, VCB_STATE_HSG ),
                                       VOL_ID_LEN )) {
            ...
            if ((XRvdVersion( RawIsoVd, VolumeFlags ) != VERSION_1) ||
                (XRvdDescType( RawIsoVd, VolumeFlags ) == VD_TERMINATOR)) {
            ...
            if (XRvdDescType( RawIsoVd, VolumeFlags ) == VD_PRIMARY) {
...

{
    ...
    if ((FlagOn( Vcb->VcbState, VCB_STATE_ISO) || VerifyVolume)) {
        ...
        while (TRUE) {
            ...
            if (!RtlEqualMemory( XIsoId, XRvdId( RawIsoVd, VCB_STATE_JOLIET ), VOL_ID_LEN ) ||
                (XRvdVersion( RawIsoVd, VCB_STATE_JOLIET ) != VERSION_1) ||
                (XRvdDescType( RawIsoVd, VCB_STATE_JOLIET ) == VD_TERMINATOR)) {
            ...
            if ((XRvdDescType( RawIsoVd, VCB_STATE_JOLIET ) == VD_SECONDARY) &&
                (RtlEqualMemory( XRvdEsc( RawIsoVd, VCB_STATE_JOLIET ),
                                 XJolietEscape[0],
                                 ESC_SEQ_LEN ) ||
                 RtlEqualMemory( XRvdEsc( RawIsoVd, VCB_STATE_JOLIET ),
                                 XJolietEscape[1],
    ...
    Vcb->Vpb->SerialNumber = XSerial32( RawIsoVd, SECTOR_SIZE );

    //
    //  Make sure the X label will fit in the Vpb.
    //
