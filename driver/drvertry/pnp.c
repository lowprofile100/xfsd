...
Abstract:
    ...
    This module implements the Plug and Play routines for XFS called by
    the dispatch driver.
...
#define BugCheckFileId                   (XFS_BUG_CHECK_PNP)
...
#ifdef ALLOC_PRAGMA
#pragma alloc_text(PAGE, XCommonPnp)
#pragma alloc_text(PAGE, XPnpCancelRemove)
#pragma alloc_text(PAGE, XPnpQueryRemove)
#pragma alloc_text(PAGE, XPnpRemove)
#pragma alloc_text(PAGE, XPnpSurpriseRemove)
#endif
...

{
    ...
    if (OurDeviceObject->DeviceObject.Size != sizeof(VOLUME_DEVICE_OBJECT) ||
        NodeType( &OurDeviceObject->Vcb ) != XFS_NTC_VCB) {
    ...
    switch ( IrpSp->MinorFunction ) {
        ...
        case IRP_MN_QUERY_REMOVE_DEVICE:
            ...
            Status = XPnpQueryRemove( IrpContext, Irp, Vcb );
            break;
        
        case IRP_MN_SURPRISE_REMOVAL:
        
            Status = XPnpSurpriseRemove( IrpContext, Irp, Vcb );
            break;

        case IRP_MN_REMOVE_DEVICE:

            Status = XPnpRemove( IrpContext, Irp, Vcb );
            break;

        case IRP_MN_CANCEL_REMOVE_DEVICE:
    
            Status = XPnpCancelRemove( IrpContext, Irp, Vcb );
            break;
...

{
    ...
    Status = XLockVolumeInternal( IrpContext, Vcb, NULL );
    ...
    if (NT_SUCCESS( Status )) {
        ...
        if (NT_SUCCESS( Status )) {
            ...
            VcbPresent = XCheckForDismount( IrpContext, Vcb, TRUE );
    ...
    //  case (in XFS) where this would actually happen, though it might still need
    //  to be implemented.
    //
...

{
    ...
    Status = XUnlockVolumeInternal( IrpContext, Vcb, NULL );
...

{
    ...
    //  seen the QUERY.
    //
    //  For XFS, this is quite easy.  In fact, we can't get a
    ...
    (VOID) XUnlockVolumeInternal( IrpContext, Vcb, NULL );
