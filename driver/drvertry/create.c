...
Abstract:
    ...
    This module implements the File Create routine for Xfs called by the
...
#define BugCheckFileId                   (XFS_BUG_CHECK_CREATE)
...
#ifdef ALLOC_PRAGMA
#pragma alloc_text(PAGE, XCommonCreate)
#pragma alloc_text(PAGE, XCompleteFcbOpen)
#pragma alloc_text(PAGE, XNormalizeFileNames)
#pragma alloc_text(PAGE, XOpenByFileId)
#pragma alloc_text(PAGE, XOpenDirectoryFromPathEntry)
#pragma alloc_text(PAGE, XOpenExistingFcb)
#pragma alloc_text(PAGE, XOpenFileFromFileContext)
#endif
...

{
    ...
    X_NAME RemainingName;
    X_NAME FinalName;
    PX_NAME MatchingName;
    ...
    if (FlagOn( IrpSp->Flags, SL_OPEN_PAGING_FILE | SL_OPEN_TARGET_DIRECTORY) ||
        ...
        XCompleteRequest( IrpContext, Irp, STATUS_ACCESS_DENIED );
    ...
    if ((FileObject->RelatedFileObject != NULL) && !OpenByFileId) {
        ...
        RelatedTypeOfOpen = XDecodeFileObject( IrpContext, RelatedFileObject, &NextFcb, &RelatedCcb );
    ...
    Status = XNormalizeFileNames( IrpContext,
                                   Vcb,
                                   OpenByFileId,
                                   IgnoreCase,
                                   RelatedTypeOfOpen,
    ...
    if ((FileName->Length == 0) &&
        ...
        XAcquireVcbShared( IrpContext, Vcb, FALSE );
    }
    ...
    try {
        ...
        if (OpenByFileId) {
            ...
            try_return( Status = XOpenByFileId( IrpContext,
                                                 IrpSp,
                                                 Vcb,
                                                 &CurrentFcb ));
        }
        ...
        if (VolumeOpen) {
            ...
            XAcquireFcbExclusive( IrpContext, CurrentFcb, FALSE );

            try_return( Status = XOpenExistingFcb( IrpContext,
                                                    IrpSp,
                                                    &CurrentFcb,
                                                    UserVolumeOpen,
                                                    FALSE,
        ...
        XAcquireFcbExclusive( IrpContext, NextFcb, FALSE );
        CurrentFcb = NextFcb;
        ...
        if (RemainingName.FileName.Length == 0) {
            ...
            if (SafeNodeType( CurrentFcb ) == XFS_NTC_FCB_DATA) {
        ...
        while (TRUE) {
            ...
            XInitializeCompoundPathEntry( IrpContext, &CompoundPathEntry );
            CleanupCompoundPathEntry = TRUE;

            FoundEntry = XFindPathEntry( IrpContext,
                                          CurrentFcb,
                                          &FinalName,
                                          IgnoreCase,
                                          &CompoundPathEntry );
            ...
            if (!FoundEntry) {
                ...
                ShortNameDirentOffset = XShortNameDirentOffset( IrpContext, &FinalName.FileName );
                ...
                if (ShortNameDirentOffset != MAXULONG) {
                    ...
                    if (CleanupFileContext) {
                        ...
                        XCleanupFileContext( IrpContext, &FileContext );
                    }

                    XInitializeFileContext( IrpContext, &FileContext );
                    CleanupFileContext = TRUE;

                    FoundEntry = XFindFileByShortName( IrpContext,
                                                        CurrentFcb,
                                                        &FinalName,
                                                        IgnoreCase,
                    ...
                    if (FoundEntry) {
                        ...
                        if (FlagOn( FileContext.InitialDirent->Dirent.DirentFlags,
                                    X_ATTRIBUTE_DIRECTORY )) {

                            XCleanupCompoundPathEntry( IrpContext, &CompoundPathEntry );
                            XInitializeCompoundPathEntry( IrpContext, &CompoundPathEntry );

                            FoundEntry = XFindPathEntry( IrpContext,
                                                          CurrentFcb,
                                                          &FileContext.InitialDirent->Dirent.XCaseFileName,
                                                          IgnoreCase,
                                                          &CompoundPathEntry );
                            ...
                            if (!FoundEntry) {
                                ...
                                XRaiseStatus( IrpContext, STATUS_FILE_CORRUPT_ERROR );
                            }
                            ...
                            if (IgnoreCase) {
                                ...
                                XUpcaseName( IrpContext, &FinalName, &FinalName );
                            }
            ...
            if (IgnoreCase && !ShortNameMatch) {
                ...
                RtlCopyMemory( FinalName.FileName.Buffer,
                               CompoundPathEntry.PathEntry.XDirName.FileName.Buffer,
                               CompoundPathEntry.PathEntry.XDirName.FileName.Length );
            }
            ...
            if (RemainingName.FileName.Length == 0) {
                ...
                try_return( Status = XOpenDirectoryFromPathEntry( IrpContext,
                                                                   IrpSp,
                                                                   Vcb,
                                                                   &CurrentFcb,
                                                                   &FinalName,
            ...
            XOpenDirectoryFromPathEntry( IrpContext,
                                          IrpSp,
                                          Vcb,
                                          &CurrentFcb,
                                          &FinalName,
        ...
        if (!FoundEntry) {
            ...
            XConvertNameToXName( IrpContext, &FinalName );

            FoundEntry = XFindFile( IrpContext,
                                     CurrentFcb,
                                     &FinalName,
                                     IgnoreCase,
                                     &FileContext,
        ...
        if (FlagOn( FileContext.InitialDirent->Dirent.Flags, X_ATTRIBUTE_DIRECTORY )) {

            XRaiseStatus( IrpContext, STATUS_DISK_CORRUPT_ERROR );
        }
        ...
        try_return( Status = XOpenFileFromFileContext( IrpContext,
                                                        IrpSp,
                                                        Vcb,
                                                        &CurrentFcb,
                                                        &FinalName,
    ...
    try_exit:  NOTHING;
        ...
        if (AbnormalTermination()) {
            ...
            if (CurrentFcb != NULL) {
                ...
                XTeardownStructures( IrpContext, CurrentFcb, &RemovedFcb );
        ...
        if (CurrentFcb != NULL) {
            ...
            XReleaseFcb( IrpContext, CurrentFcb );
        }
    ...
    if (!FlagOn( IrpContext->Flags, IRP_CONTEXT_FLAG_FULL_NAME )) {
        ...
        if (!OpenByFileId) {
            ...
            if (FileName->MaximumLength < BufferLength) {
                ...
                NewFileName.Buffer = FsRtlAllocatePoolWithTag( XPagedPool,
                                                               BufferLength,
                                                               TAG_FILE_NAME );
...

{
    ...
    try {
        ...
        if (XFidIsDirectory( FileId )) {

            TypeOfOpen = UserDirectoryOpen;
            NodeTypeCode = XFS_NTC_FCB_INDEX;

            //
            //  If the offset isn't zero then the file Id is bad.
            //

            if (XQueryFidDirentOffset( FileId ) != 0) {
        ...
        NextFcb = XLookupFcbTable( IrpContext, Vcb, FileId );
        ...
        if (NextFcb == NULL) {
            ...
            StreamOffset = XQueryFidPathTableOffset( FileId );
            ...
            XSetFidDirentOffset( ParentFileId, 0 );
            XSetFidPathTableOffset( ParentFileId, StreamOffset );
            XFidSetDirectory( ParentFileId );

            NextFcb = XLookupFcbTable( IrpContext, Vcb, ParentFileId );
            ...
            if (NextFcb == NULL) {
                ...
                while (TRUE) {
                    ...
                    Found = XLookupNextPathEntry( IrpContext,
                ...
                NextFcb = XCreateFcb( IrpContext, ParentFileId, NodeTypeCode, &Found );
                ...
                if (!Found) {
                    ...
                    XInitializeFcbFromPathEntry( IrpContext,
                                                  NextFcb,
                                                  NULL,
                                                  &CompoundPathEntry.PathEntry );
                }
            ...
            if (NextFcb != NULL) {
                ...
                if (!XAcquireFcbExclusive( IrpContext,
                                            NextFcb,
                                            TRUE )) {
                ...
                *CurrentFcb = NextFcb;
                ...
                StreamOffset = XQueryFidDirentOffset( FileId );
                ...
                if (NextFcb->FileObject == NULL) {
                    ...
                    XCreateInternalStream( IrpContext, Vcb, NextFcb );
                }
                ...
                XInitializeFileContext( IrpContext, &FileContext );
                XLookupInitialFileDirent( IrpContext,
                                           NextFcb,
                                           &FileContext,
                ...
                while (TRUE) {
                    ...
                    Found = XLookupNextInitialFileDirent( IrpContext,
                                                           NextFcb,
                                                           &FileContext );
                ...
                XLookupLastFileDirent( IrpContext, NextFcb, &FileContext );

                XLockVcb( IrpContext, Vcb );
                UnlockVcb = TRUE;

                NextFcb = XCreateFcb( IrpContext, FileId, NodeTypeCode, &Found );
                ...
                if (!Found) {
                    ...
                    XInitializeFcbFromFileContext( IrpContext,
                                                    NextFcb,
                                                    *CurrentFcb,
                                                    &FileContext );
                }
        ...
        if (!XAcquireFcbExclusive( IrpContext, NextFcb, TRUE )) {
        ...
        *CurrentFcb = NextFcb;
        ...
        if (!XIllegalFcbAccess( IrpContext,
                                 TypeOfOpen,
            ...
            Status = XCompleteFcbOpen( IrpContext,
                                        IrpSp,
                                        Vcb,
                                        CurrentFcb,
                                        TypeOfOpen,
...

{
    ...
    try {
        ...
        FileId.QuadPart = 0;
        XSetFidPathTableOffset( FileId, PathEntry->PathTableOffset );
        XFidSetDirectory( FileId );
        ...
        NextFcb = XCreateFcb( IrpContext, FileId, XFS_NTC_FCB_INDEX, &FcbExisted );
        ...
        if (!FcbExisted) {
            ...
            XInitializeFcbFromPathEntry( IrpContext, NextFcb, *CurrentFcb, PathEntry );
        }
        ...
        *CurrentFcb = NextFcb;
        ...
        XReleaseFcb( IrpContext, ParentFcb );
        ParentFcb = NULL;
...

{
    ...
    try {
        ...
        XSetFidPathTableOffset( FileId, XQueryFidPathTableOffset( (*CurrentFcb)->FileId ));
        XSetFidDirentOffset( FileId, FileContext->InitialDirent->Dirent.DirentOffset );
        ...
        NextFcb = XCreateFcb( IrpContext, FileId, XFS_NTC_FCB_DATA, &FcbExisted );
        ...
        *CurrentFcb = NextFcb;
        ...
        //
            ...
            XInsertPrefix( IrpContext,
                            NextFcb,
                            &FileContext->InitialDirent->Dirent.XFileName,
                            FALSE,
                            FALSE,
                            ParentFcb );
...

{
    ...
    if ((TypeOfOpen <= UserVolumeOpen) &&
        ...
        Status = XPurgeVolume( IrpContext, Vcb, FALSE );
        ...
        if (Vcb->VcbUserReference > XFS_RESIDUAL_USER_REFERENCE) {
    ...
    Ccb = XCreateCcb( IrpContext, Fcb, UserCcbFlags );
    ...
    XSetFileObject( IrpContext, IrpSp->FileObject, TypeOfOpen, Fcb, Ccb );
    ...
    XIncrementCleanupCounts( IrpContext, Fcb );
    XIncrementReferenceCounts( IrpContext, Fcb, 1, 1 );
