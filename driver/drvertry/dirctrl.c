...
Abstract:
    ...
    This module implements the File Directory Control routines for Xfs called
...
#define BugCheckFileId                   (XFS_BUG_CHECK_DIRCTRL)
...
#ifdef ALLOC_PRAGMA
#pragma alloc_text(PAGE, XCommonDirControl)
#pragma alloc_text(PAGE, XEnumerateIndex)
#pragma alloc_text(PAGE, XInitializeEnumeration)
#pragma alloc_text(PAGE, XNotifyChangeDirectory)
#pragma alloc_text(PAGE, XQueryDirectory)
#endif
...

{
    ...
    case IRP_MN_QUERY_DIRECTORY:
        ...
        Status = XQueryDirectory( IrpContext, Irp, IrpSp, Fcb, Ccb );
        break;

    case IRP_MN_NOTIFY_CHANGE_DIRECTORY:

        Status = XNotifyChangeDirectory( IrpContext, Irp, IrpSp, Ccb );
        break;
...

{
    ...
    default:
        ...
        XCompleteRequest( IrpContext, Irp, STATUS_INVALID_INFO_CLASS );
    ...
    try {
        ...
        XInitializeEnumeration( IrpContext,
                                 IrpSp,
                                 Fcb,
                                 Ccb,
                                 &FileContext,
        ...
        while (TRUE) {
            ...
            Found = XEnumerateIndex( IrpContext, Ccb, &FileContext, ReturnNextEntry );
            ...
            FileNameBytes = ThisDirent->XFileName.FileName.Length;
            ...
            if (((Ccb->SearchExpression.VersionString.Length != 0) ||
                 (FlagOn(ThisDirent->DirentFlags, X_ATTRIBUTE_DIRECTORY))) &&
                (ThisDirent->XFileName.VersionString.Length != 0)) {

                SeparatorBytes = 2;

                VersionStringBytes = ThisDirent->XFileName.VersionString.Length;
            }
            ...
            try {
                ...
                case FileDirectoryInformation:
                    ...
                    XConvertXTimeToNtTime( IrpContext,
                                             FileContext.InitialDirent->Dirent.XTime,
                    ...
                    if (FlagOn( ThisDirent->DirentFlags, X_ATTRIBUTE_DIRECTORY )) {
                    ...
                    //
                    //  All Xrom files are readonly.  We also copy the existence
                ...
                case FileIdBothDirectoryInformation:
                    ...
                    XSetFidFromParentAndDirent( IdBothDirInfo->FileId, Fcb, ThisDirent );
                    break;

                case FileIdFullDirectoryInformation:

                    IdFullDirInfo = Add2Ptr( UserBuffer, NextEntry, PFILE_ID_FULL_DIR_INFORMATION );
                    XSetFidFromParentAndDirent( IdFullDirInfo->FileId, Fcb, ThisDirent );
                    break;
                ...
                if (FileNameBytes != 0) {
                    ...
                    RtlCopyMemory( Add2Ptr( UserBuffer, NextEntry + BaseLength, PVOID ),
                                   ThisDirent->XFileName.FileName.Buffer,
                                   FileNameBytes );
                        ...
                        *(Add2Ptr( UserBuffer,
                        ...
                        if (VersionStringBytes != 0) {
                            ...
                            RtlCopyMemory( Add2Ptr( UserBuffer,
                                                    ...
                                                    PVOID ),
                                           ThisDirent->XFileName.VersionString.Buffer,
                                           VersionStringBytes );
                        }
                    }
                }
                ...
                if ((Status == STATUS_SUCCESS) &&
                    ...
                    //
                        ...
                        if (!XIs8dot3Name( IrpContext,
                                            ThisDirent->XFileName.FileName )) {
    
                            XGenerate8dot3Name( IrpContext,
                                                 &ThisDirent->XCaseFileName.FileName,
        ...
        //  else can block on threads in Xcreateinternalstream/purge which 
...
Routine Description:
    ...
    will never be a notify signalled on a XROM disk we still support this call.
...

{
    ...
    XAcquireVcbShared( IrpContext, IrpContext->Vcb, FALSE );
    ...
    try {
        ...
        XVerifyVcb( IrpContext, IrpContext->Vcb );
        ...
        XReleaseVcb( IrpContext, IrpContext->Vcb );
    }
...

{
    ...
    if (!FlagOn( Ccb->Flags, CCB_FLAG_ENUM_INITIALIZED )) {
        ...
        //
        //  Otherwise build the XName from the name in the stack location.
        ...
        //
            ...
            //
            //  Create a XName to check for wild cards.
            //

            WildCardName.FileName = *FileName;

            XConvertNameToXName( IrpContext, &WildCardName );
            ...
            if (WildCardName.FileName.Length == 0) {
                ...
                XRaiseStatus( IrpContext, STATUS_INVALID_PARAMETER );
            }
            ...
            SearchExpression.FileName.Buffer = FsRtlAllocatePoolWithTag( XPagedPool,
                                                                         FileName->Length,
                                                                         TAG_ENUM_EXPRESSION );
    ...
    *InitialQuery = FALSE;
    ...
    if (Fcb->FileObject == NULL) {
        ...
        XCreateInternalStream( IrpContext, Fcb->Vcb, Fcb );
    }
    ...
    if (KnownOffset) {
        ...
        XLookupInitialFileDirent( IrpContext, Fcb, FileContext, DirentOffset );
    ...
    //
        ...
        XLookupInitialFileDirent( IrpContext, Fcb, FileContext, LastDirentOffset );
        ...
        //
            ...
            do {
                ...
                Found = XLookupNextInitialFileDirent( IrpContext, Fcb, FileContext );
...

{
    ...
    while (TRUE) {
        ...
        if (ReturnNextEntry) {
            ...
            if (!XLookupNextInitialFileDirent( IrpContext, Ccb->Fcb, FileContext )) {
            ...
            XUpdateDirentName( IrpContext, ThisDirent, FlagOn( Ccb->Flags, CCB_FLAG_IGNORE_CASE ));
        ...
        if (!FlagOn( ThisDirent->DirentFlags, X_ATTRIBUTE_ASSOC )) {
            ...
            if ((PreviousDirent == NULL) ||
                (Ccb->SearchExpression.VersionString.Length != 0) ||
                (PreviousDirent->XCaseFileName.FileName.Length != ThisDirent->XCaseFileName.FileName.Length) ||
                FlagOn( PreviousDirent->DirentFlags, X_ATTRIBUTE_ASSOC ) ||
                !RtlEqualMemory( PreviousDirent->XCaseFileName.FileName.Buffer,
                                 ThisDirent->XCaseFileName.FileName.Buffer,
                                 ThisDirent->XCaseFileName.FileName.Length )) {
                ...
                if (XIsNameInExpression( IrpContext,
                                          &ThisDirent->XCaseFileName,
    ...
    if (Found) {
        ...
        XLookupLastFileDirent( IrpContext, Ccb->Fcb, FileContext );
    }
